...
// Verificar si el monto es válido o asignar todo el saldo disponible
...
// Obtener el saldo de la entidad remitente
$stmt = $pdo->prepare("SELECT saldo FROM entidades WHERE id_entidad = :id_remitente");
$stmt->execute(['id_remitente' => $id_remitente_entidad]);
$remitente = $stmt->fetch(PDO::FETCH_ASSOC);

// Si no se proporciona un monto, se toma todo el saldo de la entidad remitente
if ($monto <= 0) {
    $monto = $remitente['saldo'];
}
